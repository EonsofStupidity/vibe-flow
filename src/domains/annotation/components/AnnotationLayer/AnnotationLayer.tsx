...
export function AnnotationLayer({ enabled, slideKey }: AnnotationLayerProps) {
  ...
  useEffect(() => {
    ...
    const move = (e: PointerEvent) => {
      ...
      const p = pos(e);
      ctx.strokeStyle = "oklch(0.96 0.02 80)";
    ...
    <div className={enabled ? "pointer-events-auto" : "pointer-events-none"}>
      ...
      {enabled ? (
        <button
          type="button"
          onClick={clear}
          className="tap-target absolute bottom-24 right-6 rounded-full bg-secondary px-6 font-mono text-xs uppercase tracking-[0.2em] text-secondary-foreground hairline-b"
        >
          Clear ink
        </button>
      ) : null}
